...
document.addEventListener('DOMContentLoaded', function() {
    ...
    //Store Latest Drawn Card Data
        let $new_canvas = false;
        let $new_canvas_data ;
    //Backup Old Drawn Card Data
        let $backup_canvas = false;
        let $backup_canvas_data ;
    ...
    function handleCardSelection() {
        ...
        if (selectedType === 'combo_dog' || selectedType === 'combo_red_dog' || selectedType === 'combo_emotional_dog' || selectedType === 'combo_emotional_cat') {
    ...
    function switchInputFields(cardType) {
        ...
        } else if (cardType === 'combo_dog' || cardType === 'combo_red_dog' || cardType === 'combo_emotional_dog' || cardType === 'combo_emotional_cat') {
    ...
    function findAndLoadMatchingCards(cardType, availableCards) {
        ...
        if (cardType === 'emotional_dog') {
            console.log('Files containing "emotional":', availableCards.filter(c => c.name.toLowerCase().includes('emotional')).map(c => c.name));
            console.log('Files containing "dog":', availableCards.filter(c => c.name.toLowerCase().includes('dog')).map(c => c.name));
        } else if (cardType === 'blue_cat') {
            console.log('Files containing "cat":', availableCards.filter(c => c.name.toLowerCase().includes('cat')).map(c => c.name));
            console.log('Files containing "blue":', availableCards.filter(c => c.name.toLowerCase().includes('blue')).map(c => c.name));
        } else if (cardType === 'service_dog_handler') {
            console.log('Files containing "service":', availableCards.filter(c => c.name.toLowerCase().includes('service')).map(c => c.name));
            console.log('Files containing "handler":', availableCards.filter(c => c.name.toLowerCase().includes('handler')).map(c => c.name));
            console.log('Files containing "front":', availableCards.filter(c => c.name.toLowerCase().includes('front')).map(c => c.name));
        }
    ...
    function loadComboCards() {
        ...
        if (isRedCombo) {
            ...
            document.getElementById('comboHandlerTitle').innerHTML = '<i class="fas fa-user-tie"></i> Service Dog Handler Red';
        ...
        serviceDogHandlerFrontImg.onload = function() {
            ...
            comboEmotionalFrontCtx.drawImage(serviceDogHandlerFrontImg, 0, 0, comboEmotionalFrontCanvas.width, comboEmotionalFrontCanvas.height);
        ...
        serviceDogHandlerBackImg.onload = function() {
            ...
            comboEmotionalBackCtx.drawImage(serviceDogHandlerBackImg, 0, 0, comboEmotionalBackCanvas.width, comboEmotionalBackCanvas.height);
    ...
    function placePhotoOnFront(img) {
        ...
        const r = (currentCardType === 'blue_dog' || currentCardType === 'red_dog' || currentCardType === 'combo_dog' || currentCardType === 'emotional_dog') ? 0 : FRONT_LAYOUT.photo.radius;
        if (r > 0) {
            frontCtx.save();
            frontCtx.beginPath();
        ...
        if (currentCardType === 'child_identification' || currentCardType === 'child_identification_red') {
            ...
            if (autismNotes) {
                ...
                for (let i = 0; i < autismNotes.length; i++) {
                    ...
                    console.log(`Char ${i}: "${char}", testLine: "${testLine}", width: ${testWidth}px, maxWidth: ${currentMaxWidth}px, isFirstLine: ${isFirstLine}`);
                ...
                console.log(`Autism notes "${autismNotes}" placed as paragraph at (${notesX}, ${notesY}) with firstLineWidth: ${firstLineWidth}px`);
            }
            ...
            if (currentCardType === 'blue_dog' || currentCardType === 'red_dog' || currentCardType === 'emotional_dog' || currentCardType === 'blue_cat' || currentCardType === 'combo_dog') {
                const addressInput = document.getElementById('addressInput');
                const telephoneInput = document.getElementById('telephoneInput');
                const address = addressInput ? addressInput.value.trim() : '';
                const telephone = telephoneInput ? telephoneInput.value.trim() : '';
    ...
    function redrawFrontCanvas() {
        console.log('redrawFrontCanvas called - frontCanvas:', frontCanvas, 'frontCtx:', frontCtx, 'frontImage:', frontImage);
    ...
    function saveCard() {
        ...
        if (currentCardType === 'child_identification' || currentCardType === 'child_identification_red') {
            ...
            if (emergencyBeneficiaryCountInput) cardData.fields.emergencyBeneficiaryCount = emergencyBeneficiaryCountInput.value;
        } else if (currentCardType === 'combo_dog' || currentCardType === 'combo_red_dog' || currentCardType === 'combo_emotional_dog' || currentCardType === 'combo_emotional_cat') {
    ...
    function saveComboCards(cardData, frontImageData, backImageData) {
        ...
        .then(data => {
            if (data.success) {
                ...
                showSuccessMessage(`Combo cards saved successfully! Dog ID: ${dogData.uniqueId}, Service Dog Handler ID: ${serviceDogHandlerData.uniqueId}`);
