...
document.addEventListener("DOMContentLoaded", function() {
  ...
  function redrawComboFrontCanvas(cardType) {
    ...
    console.log(canvas.width, canvas.height, "text00");
    // Clear canvas
    ctx.clearRect(0, 0, canvas.width, canvas.height);
  ...
  function drawNamesOnComboCanvas(canvas, ctx, cardType) {
    ...
    if (animalName) {
      ...
      const fontSize = Math.min(
        ...
        maxWidth / (ctx.measureText(animalName).width / FRONT_LAYOUT.animal.baseFontPx),
      );
    ...
    if (handlerName) {
      ...
      const fontSize = Math.min(
        ...
        maxWidth / (ctx.measureText(handlerName).width / FRONT_LAYOUT.handler.baseFontPx),
      );
  ...
  function addQRCode() {
    ...
    const qrCodeUrl =
      `http://localhost/virtual_id2/final_virtual_id/New%20Cards/view_card.php?id=${window.currentCardUniqueId}`;

    // Store the unique ID for later use
    window.currentQRCodeUrl = qrCodeUrl;

    const qrSize = 60;
    const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=${qrSize}x${qrSize}&data=${
